...
class Infracao:
    ...
    def __str__(self) -> str:
        return f"[{self.artigo}] {self.colaborador} dia {self.dia:>2}: {self.detalhe}"
        ...
        for dia in sorted(dias):
            if dia + 1 in dias:
                achados.append(Infracao(
                    ...
                    f"trabalhou em {dia} e voltou em {dia + 1}, sem as 36h de descanso",
                ))
    return achados
...
def _limite_de_extras(
    ...
    for pessoa, quantos in turnos_no_mes.items():
        ...
        if horas_extras > teto_mensal:
            achados.append(Infracao(
                "art. 59", "limite de horas suplementares", pessoa, -1,
