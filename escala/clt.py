...
# Abaixo disso o arquivo não é um mês, é um recorte — e regra de teto mensal
# não se avalia em recorte. A primeira versão avaliava do mesmo jeito e
# avisava que o número estava distorcido; mostrar resultado errado com
# ressalva é pior que não mostrar, porque quem lê guarda o número e esquece a
# ressalva.
DIAS_MINIMOS_PARA_REGRA_MENSAL = 28
...
class Auditoria:
    ...
    # Falso quando o arquivo é um recorte e as regras de mês fechado ficaram de
    # fora. Quem lê o resultado precisa saber o que não foi conferido.
    regras_mensais_avaliadas: bool = True
...
def auditar(
    escala: Escala, cenario: Cenario, mes_fechado: bool | None = None
) -> Auditoria:
    """Roda todas as regras e o custo. É a única porta de entrada do módulo.

    `mes_fechado` decide se as regras que só existem no fechamento do mês — o
    teto de horas suplementares do art. 59 — entram na conta. Por omissão ele é
    inferido do tamanho do horizonte, e essa inferência é o comportamento certo
    para a maioria dos usos: quem sobe um recorte de cinco dias não quer ser
    acusado de estourar um teto mensal que ainda nem começou a correr.

    As regras diárias e semanais continuam valendo em qualquer recorte, porque
    o descanso de 36h e o repouso semanal não dependem do fechamento.
    """
    if mes_fechado is None:
        mes_fechado = cenario.dias >= DIAS_MINIMOS_PARA_REGRA_MENSAL
    ...
    return Auditoria(
        ...
        regras_mensais_avaliadas=mes_fechado,
    )
